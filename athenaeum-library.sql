...
VIEW customers AS SELECT users.id, users.username, 
users.password, users.role, users.phone_number, users
.created_at, IF(SUM(IF(penalties.payment_status =', 1, 0)) = 0, '
, ' ) AS blacklisted FROM users LEFT JOIN penalties ON users.id = penalties.borrowed_book_borrowing_customer_id 
GROUP BY users.id HAVING user 'customer'; CREATE VIEW `admin` AS 
SELECT id, username, password, role, created_at FROM users WHERE r 
'admin'; CREATE VIEW `borrowings_bookkeepings` AS 
SELECT double_entry_type, transaction_type, amount, 
borrowing_id, payment_date FROM bookkeepings WHERE 
penalty_id IS NULL AND purchasing_id IS NULL AND borrowing_id IS NOT 
NULL; CREATE VIEW `penalties_bookkeepings` AS SELECT 
double_entry_type, transaction_type, amount, purchasing_id, 
payment_date FROM bookkeepings WHERE borrowing_id IS NULL 
AND purchasing_id IS NULL AND penalty_id IS NOT NULL; 
CREATE VIEW `purchasings_bookkeepings` AS SELECT double_entry_type, 
transaction_type, amount, purchasing_id, payment_date 
FROM bookkeepings WHERE borrowing_id IS NULL AND penalty_id IS NULL 
AND purchasing_id IS NOT NULL; CREATE VIEW purchasings_details AS 
SELECT purchasings.id, purchasings.supplier_name, 
...
bookkeepings.payment_date FROM purchasings INNER JOIN purchasings_books_details ON purchasings.id = purchasings_books_details.purchasing_id 
INNER JOIN bookkeepings ON purchasings.id = bookkeepings.purchasing_id 
GROUP BY purchasings. id; /*!50003 DROP PROCEDURE IF EXISTS `checkIfSpecifiedBorrowingIsCompleted` */ 
